...
class FobiObservationApiController extends Controller
{
    ...
    private function processImageFile($file)
    {
        ...
        return $this->processAndUploadImage($file, 'uploads');
    }
    ...
    public function storeChecklistAndFauna(Request $request)
    {
        try {
            $request->validate([
                ...
                'breeding_type_id.*' => 'sometimes|nullable|array',
            ...
            Log::info('Request data dengan breeding_type_id:', $request->all());
            ...
            DB::transaction(function () use ($request, $userId, $burungnesiaUserId) {
                ...
                foreach ($faunaIds as $index => $faunaId) {
                    // Jika breeding = false, pastikan breeding_type_id = []
                    $breedingTypeId = isset($breedings[$index]) && $breedings[$index] 
                        ...
                        foreach ($request->file("images.$index") as $imageFile) {
                            ...
                            if ($result['success']) {
                                DB::table('fobi_checklist_fauna_imgs')->insert([
                                    'checklist_id' => $checklistId,
                                    'fauna_id' => $faunaId,
                                    'images' => $result['url'], // Gunakan URL dari result
        ...
        try {
            ...
            if ($startDate || $endDate) {
                if ($dateType === 'observation_date') {
                    if ($startDate) $fobiData->where('fobi_checklists.tgl_pengamatan', '>=', $startDate);
                    if ($endDate) $fobiData->where('fobi_checklists.tgl_pengamatan', '<=', $endDate);
                } else {
                    if ($startDate) $fobiData->where('fobi_checklists.created_at', '>=', $startDate);
                    if ($endDate) $fobiData->where('fobi_checklists.created_at', '<=', $endDate . ' 23:59:59');
                }
            }
            ...
            if ($request->has('grade') && is_array($request->grade)) {
                ...
                $fobiData->whereIn(DB::raw('LOWER(data_quality_assessments.grade)'), $grades);
            }

            // Filter has_media - cek dari fobi_checklist_fauna_imgs (FOBI burungnesia)
            if ($request->has('has_media') && $request->has_media) {
                $fobiData->whereExists(function($query) {
                    $query->select(DB::raw(1))
                          ...
                          ->whereColumn('fobi_checklist_fauna_imgs.fauna_id', 'fobi_checklist_faunasv1.fauna_id')
                          ->whereNotNull('fobi_checklist_fauna_imgs.images');
                });
            }

            // Filter media_type - fobi_checklist_fauna_imgs hanya punya foto
            if ($request->has('media_type') && $request->media_type) {
                if ($request->media_type === 'photo') {
                    ...
                    // FOBI burungnesia tidak punya audio di fobi_checklist_fauna_imgs
            ...
            if ($request->has('user_id') && $request->user_id) {
                $fobiData->where('fobi_checklists.fobi_user_id', $request->user_id);
            }

            // Filter taxonomy - hanya gunakan second_faunas (dari Burungnesia)
            // Catatan: Tabel faunas Burungnesia hanya berisi data burung, 
            // jadi filter untuk Plantae, Fungi, dll tidak akan mengembalikan hasil dari sini
            if ($request->has('taxonomy_value') && $request->taxonomy_value) {
                ...
                // Kategori yang tidak ada di Burungnesia (hanya berisi Aves)
                $nonBurungnesiaCategories = ['Plantae', 'Fungi', 'Mammalia', 'Reptilia', 'Amphibia', 
                    'Actinopterygii', 'Insecta', 'Lepidoptera', 'Arachnida', 'Mollusca', 'UNKNOWN'];
                if (in_array($taxonomyValue, $nonBurungnesiaCategories)) {
                    // Return empty untuk kategori non-fauna di Burungnesia
                    $fobiData->whereRaw('1 = 0');
                } else {
                    $fobiData->where(function($q) use ($taxonomyRank, $taxonomyValue) {
                        if ($taxonomyRank === 'family') {
                            $q->where('second_faunas.family', $taxonomyValue);
                        } elseif ($taxonomyRank === 'class') {
                            // Burungnesia hanya berisi Aves
                            if ($taxonomyValue === 'Aves') {
                                // Semua data di faunas adalah burung, jadi tidak perlu filter
                            } else {
                                $q->whereRaw('1 = 0'); // Tidak ada data untuk class lain
                            }
                        } elseif ($taxonomyRank === 'order') {
                            // Filter berdasarkan nameLat yang mengandung order
                            $q->where('second_faunas.nameLat', 'LIKE', "%{$taxonomyValue}%");
                        } else {
                            $q->where('second_faunas.nameLat', 'LIKE', "%{$taxonomyValue}%");
                        }
                    });
            ...
            // Filter location_name - cari berdasarkan nama lokasi
            if ($request->has('location_name') && $request->location_name) {
                $locationName = $request->location_name;
                $fobiData->where(function($q) use ($locationName) {
                    $q->whereExists(function($subq) use ($locationName) {
                        $subq->select(DB::raw(1))
                            ->from('fobi_checklist_media')
                            ->whereColumn('fobi_checklist_media.checklist_id', 'fobi_checklists.id')
            ...
            // Perbaiki query polygon untuk database tanpa fungsi spasial
            if ($request->has('polygon')) {
                ...
                $fobiData->where('fobi_checklists.latitude', '>=', $minLat)
                         ->where('fobi_checklists.latitude', '<=', $maxLat)
                         ->where('fobi_checklists.longitude', '>=', $minLng)
                         ->where('fobi_checklists.longitude', '<=', $maxLng);
            ...
            // PENTING: Gunakan LEFT JOIN ke faunas karena fauna_id bisa merujuk ke:
            // 1. faunas.id (data lama dari app)
            // 2. taxas.id (data baru dari web upload)
            // Jadi kita perlu fallback ke checklist_fauna.nama_spesies dan nama_latin
            $burungnesiaData = DB::connection('second')->table('checklists')
                ...
                ->whereNull('checklist_fauna.deleted_at');

            // Terapkan filter pencarian untuk Burungnesia
            // Cari di faunas (data lama) ATAU di checklist_fauna.nama_spesies/nama_latin (data baru)
            if ($searchQuery) {
                $burungnesiaData->where(function($query) use ($searchQuery) {
                    $query->where('faunas.nameId', 'LIKE', "%{$searchQuery}%")
                        ...
                        ->orWhere('checklist_fauna.nama_spesies', 'LIKE', "%{$searchQuery}%")
                        ->orWhere('checklist_fauna.nama_latin', 'LIKE', "%{$searchQuery}%");
                });
            }

            // Terapkan filter tanggal untuk Burungnesia berdasarkan date_type
            if ($startDate || $endDate) {
                if ($dateType === 'observation_date') {
                    if ($startDate) $burungnesiaData->where('checklists.tgl_pengamatan', '>=', $startDate);
                    if ($endDate) $burungnesiaData->where('checklists.tgl_pengamatan', '<=', $endDate);
                } else {
                    if ($startDate) $burungnesiaData->where('checklists.created_at', '>=', $startDate);
                    if ($endDate) $burungnesiaData->where('checklists.created_at', '<=', $endDate . ' 23:59:59');
                }
            }

            // Filter grade untuk Burungnesia
            // Data burungnesia selalu punya grade 'checklist burungnesia' (hardcoded)
            // Tampilkan jika user filter 'casual' atau 'checklist burungnesia'
            if ($request->has('grade') && is_array($request->grade)) {
                $grades = array_map('strtolower', $request->grade);
                if (!in_array('casual', $grades) && !in_array('checklist burungnesia', $grades)) {
                    $burungnesiaData->whereRaw('1 = 0');
                }
            }

            // Filter has_media untuk Burungnesia - cek checklist_fauna_imgs
            if ($request->has('has_media') && $request->has_media) {
                $burungnesiaData->whereExists(function($query) {
                    $query->select(DB::raw(1))
                          ->from('checklist_fauna_imgs')
                          ->whereColumn('checklist_fauna_imgs.checklist_id', 'checklists.id')
                          ->whereColumn('checklist_fauna_imgs.fauna_id', 'checklist_fauna.fauna_id')
                          ->whereNotNull('checklist_fauna_imgs.images')
                          ->whereNull('checklist_fauna_imgs.deleted_at');
                });
            }

            // Filter media_type untuk Burungnesia
            // Burungnesia hanya punya foto (images), tidak ada audio
            if ($request->has('media_type') && $request->media_type) {
                if ($request->media_type === 'photo') {
                    $burungnesiaData->whereExists(function($query) {
                        $query->select(DB::raw(1))
                    ...
                    // Burungnesia tidak punya audio, jadi exclude semua
                    $burungnesiaData->whereRaw('1 = 0');
                }
            }

            // Filter user_id untuk Burungnesia
            if ($request->has('user_id') && $request->user_id) {
                // Cari burungnesia_user_id dari fobi_users
                $fobiUser = DB::table('fobi_users')->where('id', $request->user_id)->first();
                if ($fobiUser && $fobiUser->burungnesia_user_id) {
                    $burungnesiaData->where('checklists.user_id', $fobiUser->burungnesia_user_id);
                } else {
                    $burungnesiaData->whereRaw('1 = 0');
                }
            }

            // Filter taxonomy untuk Burungnesia
            // Catatan: Burungnesia hanya berisi data burung (Aves)
            if ($request->has('taxonomy_value') && $request->taxonomy_value) {
                $taxonomyRank = $request->input('taxonomy_rank', '');
                $taxonomyValue = $request->taxonomy_value;
                
                // Kategori yang tidak ada di Burungnesia
                $nonBurungnesiaCategories = ['Plantae', 'Fungi', 'Mammalia', 'Reptilia', 'Amphibia', 
                    'Actinopterygii', 'Insecta', 'Lepidoptera', 'Arachnida', 'Mollusca', 'UNKNOWN'];
                ...
                if (in_array($taxonomyValue, $nonBurungnesiaCategories)) {
                    ...
                    $burungnesiaData->where(function($q) use ($taxonomyRank, $taxonomyValue) {
                        if ($taxonomyRank === 'family') {
                            $q->where('faunas.family', $taxonomyValue);
                        } elseif ($taxonomyRank === 'class' && $taxonomyValue === 'Aves') {
                            // Semua data Burungnesia adalah Aves, tidak perlu filter
                        } elseif ($taxonomyRank && in_array($taxonomyRank, ['genus', 'species', 'order'])) {
                            $q->where('faunas.nameLat', 'LIKE', "%{$taxonomyValue}%");
                        } else {
                            $q->where('faunas.nameLat', 'LIKE', "%{$taxonomyValue}%")
                              ->orWhere('faunas.family', 'LIKE', "%{$taxonomyValue}%");
                        }
                    });
                }
            }

            // Filter location_name untuk Burungnesia - cari di checklisttr.label
            if ($request->has('location_name') && $request->location_name) {
                $locationName = $request->location_name;
                $burungnesiaData->whereExists(function($q) use ($locationName) {
                    $q->select(DB::raw(1))
                      ->from('checklisttr')
            ...
            // Lakukan hal yang sama untuk burungnesiaData
            if ($request->has('polygon')) {
                ...
                $burungnesiaData->where('checklists.latitude', '>=', $minLat)
                               ->where('checklists.latitude', '<=', $maxLat)
                               ->where('checklists.longitude', '>=', $minLng)
                               ->where('checklists.longitude', '<=', $maxLng);
            }

            // Select statement untuk Burungnesia
            // Gunakan COALESCE untuk fallback ke checklist_fauna jika faunas tidak ada
            $burungnesiaData = $burungnesiaData->select(
                ...
                'users.id as observer_id',
                DB::raw('COALESCE(faunas.nameId, checklist_fauna.nama_spesies) as nameId'),
                DB::raw('COALESCE(faunas.nameLat, checklist_fauna.nama_latin) as nameLat'),
                'faunas.family',
            ...
            // Jika exclude_fobi=1, hanya gunakan data burungnesia murni (tanpa FOBI)
            ...
            if ($excludeFobi) {
                $allData = $burungnesiaData->sortByDesc('created_at');
            } else {
            ...
            foreach ($items as $observation) {
                if ($observation->source === 'fobi') {
                    $fobiBirdImages = DB::table('fobi_checklist_fauna_imgs')
                        ->where('checklist_id', $observation->id)
                        ->where('fauna_id', $observation->fauna_id)
                        ->select('id', 'images', 'storage_type')
                        ->get();
                    // Format URL lengkap menggunakan MediaStorageHelper
                    $observation->images = $fobiBirdImages->map(function($img) {
                        return (object)[
                            'id' => $img->id,
                            'url' => $img->images 
                    ...
                    if ($fobiBirdImages->count() > 0) {
                        $observation->has_media = 1;
                    }
                } else {
                    // Ambil gambar dari Burungnesia
                    $burungnesiaImages = DB::connection('second')
                    ...
                    $burungnesiaUrl = config('app.burungnesia_url', 'https://burungnesia.id');
                    $observation->images = $burungnesiaImages->map(function($img) use ($burungnesiaUrl) {
                        return (object)[
                            'id' => $img->id,
                            'url' => $img->images ? $burungnesiaUrl . '/storage/images_burung/' . $img->images : null
                        ];
                    });

                    if ($burungnesiaImages->count() > 0) {
                        $observation->has_media = 1;
                    }
                }
                ...
                if ($observation->source === 'burungnesia') {
                    $observation->species_count = DB::connection('second')
                        ->table('checklist_fauna')
                        ->where('checklist_id', $observation->id)
                        ->whereNull('deleted_at')
                        ->distinct('fauna_id')
                    ...
                    // Ambil location_name dari checklisttr
                    $checklisttr = DB::connection('second')
                        ->table('checklisttr')
                        ->where('checklist_id', $observation->id)
                        ->where('lang_code', 'in_ID')
                        ->first();
                    $observation->location_name = $checklisttr->label ?? '';
                } else {
                    // FOBI: hitung dari fobi_checklist_faunasv1
                    $observation->species_count = DB::table('fobi_checklist_faunasv1')
                        ->where('checklist_id', $observation->id)
                        ->distinct('fauna_id')
                        ->count('fauna_id');
    ...
    public function getUserObservations(Request $request)
    {
        try {
            ...
            foreach ($observations as $observation) {
                $fobiBirdImgs = DB::table('fobi_checklist_fauna_imgs')
                    ->where('checklist_id', $observation->id)
                    ->where('fauna_id', $observation->fauna_id)
                    ->select('id', 'images', 'storage_type')
                    ->get();
                $observation->images = $fobiBirdImgs->map(function($img) {
                    return (object)[
                        'id' => $img->id,
                        'url' => $img->images 
                            ? \App\Helpers\MediaStorageHelper::getMediaUrl(
    ...
    public function getChecklistDetail($id)
    {
        try {
            ...
            // Transform URL menggunakan MediaStorageHelper
            foreach ($medias as $media) {
                if (!filter_var($media->url, FILTER_VALIDATE_URL)) {
                    // Ambil storage_type dari database
                    $mediaRecord = DB::table('fobi_checklist_media')
                        ->where('id', $media->id ?? 0)
                        ->first();
                    
                    $media->url = MediaStorageHelper::getMediaUrl(
                        $media->url,
                        $mediaRecord->storage_type ?? 'local',
                        $media->id ?? 0
                    );
                }
            }
            ...
            $spectrograms = DB::select("
                SELECT id, spectrogram_url, storage_type
                FROM fobi_checklist_audio
                WHERE checklist_id = ? AND spectrogram_url IS NOT NULL
            ", [$id]);

            if (!empty($spectrograms)) {
                $checklist['spectrogram_url'] = MediaStorageHelper::getMediaUrl(
                    $spectrograms[0]->spectrogram_url,
                    $spectrograms[0]->storage_type ?? 'local',
                    $spectrograms[0]->id
                );
            }
