...
class ChecklistDetailController extends Controller
{
    ...
    public function getMediaComments($mediaId)
    {
        try {
            ...
            // Tentukan tabel yang akan digunakan untuk komentar
            $commentsTable = $source === 'kupunesia' 
                ? 'fobi_checklist_fauna_imgs_kupnes_comments' 
                : 'fobi_checklist_fauna_imgs_comments';
            
            // Deteksi apakah media berasal dari aplikasi atau database utama FOBi
            $isAppMedia = false;
            
            if ($source === 'kupunesia') {
                // Cek apakah media ada di tabel FOBi
            ...
            // Jika media dari aplikasi, gunakan ID yang sama di tabel komentar FOBi
            // Ini mengasumsikan bahwa ID dari aplikasi dipertahankan saat disimpan di tabel komentar
            
            $comments = DB::table($commentsTable . ' as c')
    ...
    public function addMediaComment(Request $request, $mediaId)
    {
        try {
            ...
            // Kita selalu menyimpan komentar di database FOBi terlepas dari asal medianya
    ...
    public function getMediaRating($mediaId)
    {
        try {
            ...
            // Tentukan tabel yang akan digunakan
            $ratingsTable = $source === 'kupunesia' 
                ? 'fobi_checklist_fauna_imgs_kupnes_ratings' 
                : 'fobi_checklist_fauna_imgs_ratings';
            ...
            $ratingStats = DB::table($ratingsTable)
                ->select([
            ...
            $userRating = null;
            if ($userId) {
                $userRating = DB::table($ratingsTable)
                    ->where([
                        'media_id' => $mediaId,
                        'user_id' => $userId
                    ])
    ...
    public function rateMedia(Request $request, $mediaId)
    {
        try {
            ...
            $existingRating = DB::table($ratingsTable)
                ->where([
                    'media_id' => $mediaId,
                    'user_id' => $user->id
                ])
