...
class TaxaController extends Controller
{
        ...
        try {
            ...
            $taxaQuery = Taxa::where(function($q) use ($query, $cleanQuery) {
                ...
                $q->where('scientific_name', 'LIKE', "%{$query}%")
                  ...
                  ->orWhere('cname_genus', 'LIKE', "%{$query}%");
                ...
                if ($query !== $cleanQuery) {
                    $q->orWhere('scientific_name', 'LIKE', "%{$cleanQuery}%")
                      ...
                      ->orWhere('cname_genus', 'LIKE', "%{$cleanQuery}%");
                }
            })
            ->where('status', 'active')
    ...
    public function generateSqliteDatabase()
    {
        try {
            ...
            do {
                ...
                $stmt = $pdo->prepare('
                    INSERT INTO taxa (
                        id, scientific_name, common_name, cname_species, cname_genus, taxon_rank,
    ...
    /**
     * Pencarian taxa ringan untuk Kupunesia suggestion (kupu-kupu/Lepidoptera)
     * Endpoint: GET /api/taksa/search/kupunesia
     * Optimasi: Query langsung dengan LIMIT, filter order Lepidoptera
     */
    public function searchKupunesia(Request $request)
    {
        try {
            $query = $request->get('q', $request->get('query', ''));
            $limit = min(intval($request->get('limit', 10)), 50);
            ...
            Log::info("searchKupunesia query: {$query}, limit: {$limit}");
            
            // Query untuk kupu-kupu (order Lepidoptera)
            $taxaQuery = Taxa::where(function($q) use ($query) {
                    $q->where('scientific_name', 'LIKE', "{$query}%")
                ...
                ->where('status', 'active')
                ->where('taxonomic_status', 'ACCEPTED')
                ->where('order', 'Lepidoptera'); // Filter untuk kupu-kupu
            
            // Ambil langsung dengan LIMIT - sorting di database
            $taxa = $taxaQuery
                ->select([
                    'id',
                    'scientific_name',
                ...
                ->orderByRaw("CASE WHEN scientific_name LIKE ? THEN 0 ELSE 1 END", ["{$query}%"])
                ->orderBy('scientific_name')
                ->limit($limit)
                ->get();
            
            // Format response untuk suggestion
            $data = $taxa->map(function($item) {
                // Fallback common_name: cname_species -> Cname -> Cname_two -> ... -> Cname_ten
                $commonName = $item->cname_species 
                    ?? $item->Cname 
                    ?? $item->Cname_two 
                    ?? $item->Cname_three 
                    ?? $item->Cname_four 
            ...
            Log::error('Error in searchKupunesia: ' . $e->getMessage());
    ...
    /**
     * Pencarian taxa ringan untuk Burungnesia suggestion (burung/Aves)
     * Endpoint: GET /api/taksa/search/burungnesia
     * Optimasi: Query langsung dengan LIMIT, filter class Aves
     */
    public function searchBurungnesia(Request $request)
    {
        try {
            $query = $request->get('q', $request->get('query', ''));
            $limit = min(intval($request->get('limit', 10)), 50);
            ...
            Log::info("searchBurungnesia query: {$query}, limit: {$limit}");
            
            // Abaikan tanda "-" dalam pencarian (contoh: "ayam-hutan" -> "ayam hutan")
            $queryNormalized = str_replace('-', ' ', $query);
            
            // Query untuk burung (class Aves)
            $taxaQuery = Taxa::where(function($q) use ($query, $queryNormalized) {
                    $q->where('scientific_name', 'LIKE', "{$query}%")
                      ->orWhere('scientific_name', 'LIKE', "% {$query}%")
                      ->orWhere('Cname', 'LIKE', "%{$query}%")
                      ->orWhere('cname_species', 'LIKE', "%{$query}%")
                      // Pencarian di cname_genus, cname_family untuk menemukan genus/family dengan nama lokal
                      ->orWhere('cname_genus', 'LIKE', "%{$query}%")
                      ->orWhere('cname_family', 'LIKE', "%{$query}%")
                      // Pencarian dengan tanda "-" dihilangkan
                      ->orWhereRaw("REPLACE(Cname, '-', ' ') LIKE ?", ["%{$queryNormalized}%"])
                      ->orWhereRaw("REPLACE(cname_species, '-', ' ') LIKE ?", ["%{$queryNormalized}%"])
                      ->orWhereRaw("REPLACE(cname_genus, '-', ' ') LIKE ?", ["%{$queryNormalized}%"])
                      ->orWhereRaw("REPLACE(cname_family, '-', ' ') LIKE ?", ["%{$queryNormalized}%"]);
                })
                ->where('status', 'active')
                ->where('taxonomic_status', 'ACCEPTED')
                ->where('class', 'Aves') // Filter untuk burung
                ->whereNotIn('taxon_rank', ['subspecies', 'variety', 'form']); // Hide subspecies
            ...
            // Format response untuk suggestion
            $data = $taxa->map(function($item) use ($query) {
                $rank = $item->rank ?? 'species';
                
                // Common name berdasarkan rank
                $commonName = '';
                $cnameGenus = $item->cname_genus ?? '';
                $cnameFamily = $item->cname_family ?? '';
                $cnameOrder = $item->cname_order ?? '';
                
                switch ($rank) {
                    case 'species':
                        $commonName = $item->cname_species ?? $item->Cname ?? '';
                        break;
                    case 'genus':
                        $commonName = $item->cname_genus ?? $item->Cname ?? '';
                        // Jika cname_genus kosong, coba ambil dari species yang ada di genus ini
                        if (empty($commonName) && !empty($item->scientific_name)) {
                            $speciesWithCname = Taxa::where('genus', $item->scientific_name)
                                ->where('taxon_rank', 'species')
                                ->whereNotNull('cname_genus')
                                ->where('cname_genus', '!=', '')
                                ->first(['cname_genus']);
                            if ($speciesWithCname) {
                                $commonName = $speciesWithCname->cname_genus;
                                $cnameGenus = $speciesWithCname->cname_genus;
                            }
                        }
                        break;
                    case 'family':
                        $commonName = $item->cname_family ?? $item->Cname ?? '';
                        break;
                    case 'order':
                        $commonName = $item->cname_order ?? $item->Cname ?? '';
                        break;
                    default:
                        $commonName = $item->Cname ?? '';
                        break;
                }
                
                // Untuk nama latin: gunakan genus + species jika rank adalah species
                $scientificName = $item->scientific_name;
                if ($rank === 'species' && !empty($item->species)) {
                    $genus = $item->genus ?? '';
                ...
                return [
                    ...
                    'scientific_name' => $scientificName,
                    'common_name' => $commonName,
                    'common_name_genus' => $cnameGenus,
                    'common_name_family' => $cnameFamily,
                    'common_name_order' => $cnameOrder,
                    'rank' => $rank,
                    'family' => $item->family ?? '',
                    'image_url' => null,
                ];
            ...
            Log::error('Error in searchBurungnesia: ' . $e->getMessage());
