...
class GenusGalleryController extends Controller
{
    ...
    public function getGenusDistribution($taxaId)
    {
        try {
            ...
            // Find all descendant species for this genus using the same logic as TaxaDetailController
            ...
            $fobiLocations = DB::table('fobi_checklist_taxas as fct')
                ->distinct()
                ->where(function($query) use ($taxaId, $descendantSpecies) {
            ...
            if (in_array(strtolower($classData), ['aves'])) {
                ...
                try {
                    // Find descendant bird species for Burungnesia
                    $descendantBirdSpecies = DB::table('taxas')
                        ->where('genus', $genus)
                        ->where('taxon_rank', 'species')
                        ->where('class', 'Aves') // Only birds for Burungnesia
                        ->whereNotNull('genus')
                        ->pluck('scientific_name')
                        ->toArray();
                    
                    if (!empty($descendantBirdSpecies)) {
                        $burungnesiaQuery = DB::connection('second')
                            ...
                            ->where(function($query) use ($descendantBirdSpecies) {
                                foreach ($descendantBirdSpecies as $speciesName) {
                                    $query->orWhere('faunas.nameLat', 'LIKE', $speciesName . '%');
                                }
                            });
                    } else {
                        // No descendant bird species found, return empty collection
                        $burungnesiaLocations = collect([]);
                        $allLocations = $allLocations->concat($burungnesiaLocations);
                        \Log::info('No descendant bird species found for genus:', ['genus' => $genus]);
                        // Skip to next section
                        goto skipBurungnesia;
                    }
            ...
            if (in_array(strtolower($classData), ['insecta', 'lepidoptera'])) {
                ...
                try {
                    // Find descendant insect species for Kupunesia
                    $descendantInsectSpecies = DB::table('taxas')
                        ->where('genus', $genus)
                        ->where('taxon_rank', 'species')
                        ->whereIn('class', ['Insecta']) // Only insects for Kupunesia
                        ->whereNotNull('genus')
                        ->pluck('scientific_name')
                        ->toArray();
                    
                    if (!empty($descendantInsectSpecies)) {
                        $kupunesiaQuery = DB::connection('third')
                            ->table('checklist_fauna')
                            ->distinct()
                            ->join('checklists', 'checklist_fauna.checklist_id', '=', 'checklists.id')
                            ->join('faunas', 'checklist_fauna.fauna_id', '=', 'faunas.id')
                            ->where(function($query) use ($descendantInsectSpecies) {
                                foreach ($descendantInsectSpecies as $speciesName) {
                                    $query->orWhere('faunas.nameLat', 'LIKE', $speciesName . '%');
                                }
                            });
                    } else {
                        // No descendant insect species found, return empty collection
                        $kupunesiaLocations = collect([]);
                        $allLocations = $allLocations->concat($kupunesiaLocations);
                        \Log::info('No descendant insect species found for genus:', ['genus' => $genus]);
                        // Skip to final section
                        goto skipKupunesia;
                    }
            ...
            \Log::info('Genus distribution data:', [
                ...
                'valid_locations' => $validLocations->count(),
