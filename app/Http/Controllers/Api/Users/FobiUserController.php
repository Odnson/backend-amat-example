...
class FobiUserController extends Controller
{
    public function register(Request $request)
    {
        try {
            ...
            $validatedData = $request->validate([
                'fname' => 'nullable|max:20',
                'lname' => 'nullable|max:20',
            ...
            try {
                ...
                // Ambil data dari database eksternal jika tersedia
                $fname = $request->fname;
                $lname = $request->lname;
                $phone = $request->phone ?: '';
                $organization = $request->organization ?: '';

                // Prioritas: Burungnesia > Kupunesia > Request data
                if ($burungnesiaUser) {
                    $fname = $fname ?: ($burungnesiaUser->fname ?? '');
                    $lname = $lname ?: ($burungnesiaUser->lname ?? '');
                    $phone = $phone ?: ($burungnesiaUser->phone ?? '');
                    $organization = $organization ?: ($burungnesiaUser->organization ?? '');
                }
                
                if ($kupunesiaUser) {
                    $fname = $fname ?: ($kupunesiaUser->fname ?? '');
                    $lname = $lname ?: ($kupunesiaUser->lname ?? '');
                    $phone = $phone ?: ($kupunesiaUser->phone ?? '');
                    $organization = $organization ?: ($kupunesiaUser->organization ?? '');
                }
        ...
        try {
            ...
            if (!$token = JWTAuth::attempt($credentials)) {
                return response()->json([
                    ...
                    'message' => 'Username/Email atau password yang Anda masukkan tidak sesuai. Silakan periksa kembali.'
                ], 401);
            }
            ...
            // Cek verifikasi email FOBI (utama)
            if (!$user->email_verified_at) {
                ...
                return response()->json([
                    ...
                    'message' => 'Email Anda belum diverifikasi. Silakan cek email untuk link verifikasi atau kirim ulang verifikasi.',
                    'user_friendly_message' => 'Akun Anda belum diverifikasi. Silakan cek email Anda untuk menyelesaikan verifikasi.',
                    'verificationStatus' => $verificationStatus,
                    'email' => $user->email
                ], 403);
            }

            // Cek status approval - handle null/empty is_approved
            if (!$user->is_approved || $user->is_approved == 0) {
                \Log::info('Login blocked - account not approved:', [
                    'user_id' => $user->id,
                    'email' => $user->email,
                ...
                return response()->json([
                    ...
                    'message' => 'Akun Anda belum disetujui oleh administrator. Silakan tunggu persetujuan atau hubungi admin.',
                    'user_friendly_message' => 'Akun Anda sedang dalam proses persetujuan. Silakan tunggu konfirmasi dari administrator.',
                    'verificationStatus' => $verificationStatus,
                    'email' => $user->email
                ], 403);
            }
            ...
            $profilePictureUrl = $this->getProfilePictureUrl($user->profile_picture);
            ...
            return response()->json([
                ...
                'user' => [
                    ...
                    'license_observation' => $user->license_observation,
                    'license_photo' => $user->license_photo,
                    'license_audio' => $user->license_audio,
                    'verificationStatus' => $verificationStatus
                ]
            ]);
            ...
            return response()->json([
                ...
                'message' => 'Terjadi kesalahan saat membuat token autentikasi. Silakan coba lagi.'
            ], 500);
        } catch (\Exception $e) {
            ...
            return response()->json([
                'error' => 'LOGIN_FAILED',
                'message' => 'Terjadi kesalahan saat login. Silakan coba lagi atau hubungi administrator jika masalah berlanjut.'
            ], 500);
        }
    }
    ...
    public function verifyEmail($token, $type)
    {
        try {
            ...
            // Update status approval jika email utama FOBI sudah diverifikasi
            if ($user->email_verified_at) {
                $user->is_approved = 1;
            }
    ...
    public function getUserProfile($id)
    {
        try {
            ...
            // SECURED: Tidak expose email dan phone di endpoint publik
            $user = FobiUser::select(
                'id',
                'fname',
                'lname',
                'uname',
                // 'email', // REMOVED: Data sensitif
                // 'phone', // REMOVED: Data sensitif
                'organization',
                'bio',
                'profile_picture',
                'license_observation',
    ...
    private function getProfilePictureUrl($path)
    {
        ...
        try {
            // Coba ambil dari S3 terlebih dahulu jika S3 tersedia
            if ($disk === 's3' && $s3Available) {
                try {
                    if (Storage::disk('s3')->exists($cleanPath)) {
                        return Storage::disk('s3')->url($cleanPath);
                    }
                    \Log::debug('Profile picture not found in S3, trying local:', ['path' => $cleanPath]);
                } catch (\Exception $e) {
                    \Log::debug('S3 check failed, trying local:', ['error' => $e->getMessage()]);
                }
            }
            ...
            // Jika tidak ada di keduanya, kembalikan URL berdasarkan disk yang dikonfigurasi
            if ($disk === 's3' && $s3Available) {
                return Storage::disk('s3')->url($cleanPath);
            }
